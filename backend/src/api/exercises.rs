...
use crate::{
    domain::planning::ExerciseType, dto::planning::{ExerciseDTO, ExerciseTypeDTO, PlannedExerciseDTO}, errors::{ExerciseError, PlannedExerciseError}, persistence::{exercises as db, sqlite}
};

#[frb(sync)]
pub fn create_exercise(name: String, exercise_type: ExerciseTypeDTO) -> Result<ExerciseDTO, ExerciseError> {
...
#[frb(sync)]
pub fn create_planned_exercise(workout_id: i64, exercise_id: i64) -> Result<PlannedExerciseDTO, PlannedExerciseError> {
    let conn = sqlite::init_db("structure.db")?;
...
pub fn get_planned_exercise(id: i64) -> Result<PlannedExerciseDTO, PlannedExerciseError> {
    ...
    let planned_exercise = db::get_planned_exercise(&conn, id)?.ok_or(PlannedExerciseError::NotFound { id })?;

    Ok(PlannedExerciseDTO::from(&planned_exercise))
}

#[frb(sync)]
pub fn list_planned_exercises(workout_id: i64) -> Result<Vec<PlannedExerciseDTO>, PlannedExerciseError> {
    let conn = sqlite::init_db("structure.db")?;

    let planned_exercises = db::list_planned_exercises(&conn, workout_id)?;

    Ok(planned_exercises.iter().map(PlannedExerciseDTO::from).collect())
}